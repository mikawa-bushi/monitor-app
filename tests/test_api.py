...
def setup_test_data():
    ...
    db.session.execute(
        text(
            "INSERT INTO products (id, name, price) VALUES (2, 'Another Product', 200.0)"
        )
    )
    ...
    db.session.execute(
        text(
            "INSERT INTO orders (id, user_id, product_id, amount) VALUES (1, 1, 1, 5.0)"
        )
    )
...
class TestProductsAPI:
    ...
    def test_create_product(self, client):
        ...
        new_product = {"name": "New Product", "price": 150.5}
    ...
    def test_update_product(self, client):
        ...
        updated_data = {"name": "Updated Product", "price": 250.0}
...
class TestOrdersAPI:
    ...
    def test_create_order(self, client):
        ...
        new_order = {"user_id": 1, "product_id": 2, "amount": 3.0}

        response = client.post(
            "/api/orders", data=json.dumps(new_order), content_type="application/json"
        )
        assert response.status_code == 201
...
class TestErrorHandling:
    ...
    def test_missing_content_type(self, client):
        ...
        response = client.post("/api/users", data=json.dumps({"name": "test"}))
...
class TestDataValidation:
    ...
    def test_create_order_with_foreign_keys(self, client):
        ...
        # 存在するユーザーと商品での注文作成
        order_data = {"user_id": 1, "product_id": 1, "amount": 2.5}

        response = client.post(
            "/api/orders", data=json.dumps(order_data), content_type="application/json"
        )
        assert response.status_code == 201
